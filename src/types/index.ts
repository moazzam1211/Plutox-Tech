 ...
 *
 * Generated by `scripts/import-downloads.mjs` from electron-builder's manifest —
 * see `src/data/downloads.ts`. `href` is absolute because the binaries are served
 * from the update host, not from this site.
 */
export interface PosBuild {
  slug: string;
  name: string;
  tagline: string;
  blurb: string;

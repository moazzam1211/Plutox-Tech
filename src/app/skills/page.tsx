  ...
  return (
    <>
      ...
      <PageHeader
        ...
        lede="Not a list of things learned once and delegated since. Every one of these is used on live client work — every one of the seven platforms is built with them."
      >
        <StatStrip
              ...
              <RevealItem key={group.label} className="h-full">
                <Panel className="h-full p-6">
                  <div className="flex items-center gap-3 border-b border-border pb-4">
                    ...
                    <span className="ml-auto font-mono text-[0.75rem] whitespace-nowrap text-muted-foreground">
                  ...
                  <ul className="mt-4 flex flex-wrap gap-1.5">
                    {group.skills.map((skill) => (
                      <li
                        key={skill}
                        className="rounded border border-border px-2 py-1 font-mono text-[0.75rem] text-muted-foreground"
                      >
                        {skill}
                      </li>
                    ))}
              ...
              <Reveal key={category} preset="fadeUp">
                <div>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-mono text-[0.75rem] whitespace-nowrap text-muted-foreground/70">
                      {items.length} tools
                    </span>
                  </div>
                  ...
                  <ul className="mt-4 flex flex-wrap gap-2">
                    {items.map((tech) => (
                      ...
                      >
                        <span
                          className="grid size-6 shrink-0 place-items-center rounded font-mono text-[0.75rem] font-medium"
                          style={{
                            color: tech.color,
